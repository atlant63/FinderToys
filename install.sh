...
if ! curl -fsSL -o "$DMG_PATH" "$DOWNLOAD_URL" 2>/dev/null; then
    # Fallback to direct release asset tag v2.0.0 or raw
    DOWNLOAD_URL="https://github.com/$REPO/releases/download/v2.0.0/FinderToys.dmg"
    if ! curl -fsSL -o "$DMG_PATH" "$DOWNLOAD_URL" 2>/dev/null; then
        echo "  ℹ️  Fetching from repository..."
        curl -fsSL -o "$DMG_PATH" "https://raw.githubusercontent.com/$REPO/main/dist/MacNewFile.dmg" 2>/dev/null || true
    fi
fi
...
cp -R "$APP_SOURCE" "/Applications/MacNewFile.app"
...
echo "  2. System Settings -> General -> Login Items & Extensions: Ensure Finder Extension is ON."
echo "  -------------------------------------------------"
